...
**Searching Nodes:**
Directly above the navigation tree on the left, you will find the search bar. Typing keywords here will search your current vault and return the 20 highest-scoring matches. You can quickly cycle through these top results using the arrow buttons next to the search bar.
...
To select a node, look at the navigation tree. Hover your mouse over a node's icon (e.g., the folder or document icon) until it turns into a checkbox, then click it. You can select single nodes, or `Shift + Click` to select multiple nodes at once.
...
*   **Print Entire Vault:** If you want a complete physical backup or to read your entire knowledge base offline, click "Print Entire Vault". This will sequentially render every single node in your currently active vault into one massive, seamlessly readable document.